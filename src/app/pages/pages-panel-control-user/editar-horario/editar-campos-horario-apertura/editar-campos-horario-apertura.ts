...
export class EditarCamposHorarioApertura implements OnInit {
  ...
  daySchedules: { [key: string]: OpeningHour[] } = {};
  loading = false;
  ...
  loadSchedules() {
    this.openingScheduleService.getOpeningHoursByBusiness(this.businessId).subscribe({
      next: (hours) => {
  ...
  createDefaultSchedule(day: string): OpeningHour {
    return {
  ...
  removeScheduleFromDay(day: string, schedule: OpeningHour) {
    if (!this.daySchedules[day]) {
      this.daySchedules[day] = [];
    }
    const index = this.daySchedules[day].indexOf(schedule);
  ...
  deleteAllSchedules() {
    ...
    this.openingScheduleService.deleteAllOpeningHours(this.businessId).subscribe({
      next: () => {
        this.initializeEmptySchedules();
